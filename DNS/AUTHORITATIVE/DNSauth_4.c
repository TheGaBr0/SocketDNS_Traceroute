...
int main()
{
	...
	writeToFile("DNSauthoritative-4  started\n");

	snprintf(logoutput, sizeof(logoutput), "DNSauthoritative-4 listening on port: %d\n", PORT);
	...
	*initialized = 0;
	...
	while (1)
	{
		...
		{
			writeToFile("DNSauthoritative-4  client accepted\n");
			if (pid = fork() == 0)
			{	
...
{	
	...
	if(strcmp(bufferInput, "INIT_COMPLETED") == 0){
		writeToFile("DNSauthoritative-4 delegation completed\n");
		*initialized = 1;
		send = write(sd, "AUTH-4: INIT OK", MAXLEN);
		if (send < 0)
	...
	if(!*initialized){
		writeToFile("DNSauthoritative-4 receving delegation from TLD\n");
		getDelegationFromTLD(localDatabase, bufferInput);
		send = write(sd, "AUTH-4: Entry added", MAXLEN);
		if (send < 0)
			die("address send() error");
	}
	else{
		found = false;
		snprintf(logoutput, sizeof(logoutput), "DNSauthoritative-4 data received from TLD: %s\n", bufferInput);
		writeToFile(logoutput);
		for(i = 0; i<=localDatabase->length; i++){
			if(strcmp(bufferInput, localDatabase->domains[i]) == 0){
				found = true;
				snprintf(logoutput, sizeof(logoutput), "DNSauthoritative-4 sending response to TLD: %s\n", localDatabase->addresses[i]);
				writeToFile(logoutput);
		...
		if(!found){
			...
			writeToFile("DNSauthoritative-4 sending response to TLD: NOT_EXISTS");
			if (send < 0)
