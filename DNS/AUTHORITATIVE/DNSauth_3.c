...
int main()
{
	...
	writeToFile("DNSauthoritative-3  started\n");

	snprintf(logoutput, sizeof(logoutput), "DNSauthoritative-3 listening on port: %d\n", PORT);
	...
	*initialized = 0;
	...
	while (1)
	{
		...
		{
			writeToFile("DNSauthoritative-3  client accepted\n");
			if (pid = fork() == 0)
			{	
...
{	
	...
	if(strcmp(bufferInput, "INIT_COMPLETED") == 0){
		writeToFile("DNSauthoritative-3 delegation completed\n");
		*initialized = 1;
		send = write(sd, "AUTH-3: INIT OK", MAXLEN);
		if (send < 0)
	...
	if(!*initialized){
		writeToFile("DNSauthoritative-3 receving delegation from TLD\n");
		getDelegationFromTLD(localDatabase, bufferInput);
		send = write(sd, "AUTH-3: Entry added", MAXLEN);
		if (send < 0)
			die("address send() error");
	}
	else{
		found = false;
		snprintf(logoutput, sizeof(logoutput), "DNSauthoritative-3 data received from TLD: %s\n", bufferInput);
		writeToFile(logoutput);
		for(i = 0; i<=localDatabase->length; i++){
			if(strcmp(bufferInput, localDatabase->domains[i]) == 0){
				found = true;
				snprintf(logoutput, sizeof(logoutput), "DNSauthoritative-3 sending response to TLD: %s\n", localDatabase->addresses[i]);
				writeToFile(logoutput);
		...
		if(!found){
			...
			writeToFile("DNSauthoritative-3 sending response to TLD: NOT_EXISTS");
			if (send < 0)
