...
int main()
{
	...
	snprintf(logoutput, sizeof(logoutput), "DNStld-2 listening on port: %d\n", PORT);
	...
	while (1)
	{
		...
		{
			writeToFile("DNStld-2 client accepted\n");
...
void getClientResponse(int sd, char *bufferInput)
{
	...
	snprintf(logoutput, sizeof(logoutput), "DNStld-2 data received from DNSRoot: %s\n", bufferInput);
	writeToFile(logoutput);
}
...
void sendClientResponse(int sd, char *bufferOutput)
{
	...
	snprintf(logoutput, sizeof(logoutput), "DNStld-2 sending response to DNSRoot: %s\n", bufferOutput);
	writeToFile(logoutput);
...
void forwardToAuthServer(int serverport, char *bufferInput, char *bufferOutput, bool log)
{
	...
	if(log){
		snprintf(logoutput, sizeof(logoutput), "DNStld-2 asking for the address: %s to SLD on port %d\n", bufferInput, serverport);
		writeToFile(logoutput);
	}else{
	...
	if(log){
		snprintf(logoutput, sizeof(logoutput), "DNStld-2 data received from SLD: %s\n", bufferOutput);
		writeToFile(logoutput);
	}else{
...
void delegateAuthoritatives(char *bufferInput, char *bufferOutput)
{
	...
	writeToFile("DNStld-2 delegation to authoritatives started\n");
	...
	writeToFile("DNStld-2 delegation to authoritatives completed\n");
...
void getDelegationFromRoot(char *bufferInput, int sd, struct sockaddr_in client_ip_port, int client_ip_port_length)
{
	...
	writeToFile("DNStld-2 receving delegation from root\n");
	...
	{	
		...
		while(1){
			...
			if (strcmp(bufferInput, "INIT_DEL_COMPLETED") == 0)
			{
				...
				del_db = true;
				send = write(new_socket, "TLD-2: INIT DEL OK", MAXLEN);
				if (send < 0)
			...
			if (strcmp(bufferInput, "INIT_TLD_COMPLETED") == 0)
			{
				...
				tld_db = true;
				send = write(new_socket, "TLD-2: INIT TLD OK", MAXLEN);
				if (send < 0)
					die("address send() error");
				continue;
			}
			...
			if(!del_db){
				...
				send = write(new_socket, "TLD-2: FILL DEL OK", MAXLEN);
				if (send < 0)
					die("address send() error");
				j++;
			}else{
				if(!tld_db){
					...
					send = write(new_socket, "TLD-2: FILL TLD OK", MAXLEN);
					if (send < 0)
						die("address send() error");
				}
	...
	writeToFile("DNStld-2 delegation completed\n");
}
